...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="post" action="/dashboard/wisata/{{$data->id}}" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('lokasi') is-invalid @enderror" name="lokasi" id="lokasi" rows="3">{{{ old('lokasi', $data->lokasi) }}}</textarea>
                @error('lokasi')
                    <div class="invalid-feedback">
                        {{$message}}
                    </div>
